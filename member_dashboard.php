    ...
    if (!empty($title) && !empty($content)) {
        ...
        if (isset($_FILES['image']) && $_FILES['image']['error'] === UPLOAD_ERR_OK) {
            ...
            if ($file_size > $max_file_size) {
                ...
                // Secure filename and move file
                $safe_filename = uniqid() . '_' . preg_replace('/[^A-Za-z0-9\-\.]/', '', $file_name);
                $target_file = $upload_dir . $safe_filename;

                if (move_uploaded_file($file_tmp, $target_file)) {
                    $image_url = 'uploads/blog/' . $safe_filename;
                }
            }
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    if (!empty($first_name) && !empty($last_name)) {
        ...
        if (isset($_FILES['avatar']) && $_FILES['avatar']['error'] === UPLOAD_ERR_OK) {
            ...
            $allowed_types = ['image/jpeg', 'image/png', 'image/gif', 'image/webp'];
            $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
            $max_file_size = 2 * 1024 * 1024; // 2MB for avatar
            ...
            if ($file_size > $max_file_size) {
                ...
                $errors[] = 'Only JPG, PNG, GIF, and WebP images are allowed for avatar';
            } elseif (!getimagesize($file_tmp)) {
                $errors[] = 'Invalid avatar image file';
            } else {
                // Secure filename and move file
                $safe_filename = uniqid() . '_' . preg_replace('/[^A-Za-z0-9\-\.]/', '', $file_name);
                $target_file = $upload_dir . $safe_filename;
                ...
                if (move_uploaded_file($file_tmp, $target_file)) {
                    $avatar_url = 'uploads/avatars/' . $safe_filename;
                }
            }
        }
...
// Get user's posts
$user_posts = $db->select("SELECT * FROM blog_posts WHERE author_id = ? ORDER BY created_at DESC", [$user_id]);
