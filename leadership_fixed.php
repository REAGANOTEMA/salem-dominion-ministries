<?php
// Fixed Leadership Page - No Database Errors
...
// Initialize leadership data with fallback
$leadership = [
    [
...
// Try to get leadership data from database (optional)
try {
    ...
    // Create table
    $createTableSQL = "CREATE TABLE IF NOT EXISTS leadership (
    ...
    $result = $conn->query("SELECT COUNT(*) as count FROM leadership");
    if ($result === false) {
        throw new Exception("Count query failed: " . $conn->error);
    }
    
    $row = $result->fetch_assoc();
    $leadership_count = $row['count'] ?? 0;
    $result->free();
    
    if ($leadership_count == 0) {
        // Insert leadership data
        foreach ($leadership as $leader) {
            $stmt = $conn->prepare("INSERT INTO leadership (name, title, bio, email, phone, image, order_position) VALUES (?, ?, ?, ?, ?, ?, ?)");
            if ($stmt === false) {
    ...
    $result = $conn->query("SELECT * FROM leadership WHERE is_active = 1 ORDER BY order_position ASC");
    if ($result === false) {
    ...
    if (!empty($db_leadership)) {
        $leadership = $db_leadership;
    }
    ...
    error_log("Leadership database error: " . $e->getMessage());
