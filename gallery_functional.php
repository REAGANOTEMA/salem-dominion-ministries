        ...
        if (in_array($file['type'], $allowed_types) && in_array($file_extension, $allowed_extensions)) {
            ...
            if (move_uploaded_file($file['tmp_name'], $filepath)) {
                try {
                    ...
                    $upload_error = 'Database error: Failed to save image information.';
                }
            } else {
...
// Handle image deletion (for logged-in users who uploaded the image)
$delete_success = '';
$delete_error = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_image']) && isset($_SESSION['user_id'])) {
    ...
    try {
        ...
        $image_info = $db->selectOne("SELECT * FROM gallery WHERE id = ?", [$image_id]);
        
        if ($image_info && ($image_info['uploaded_by'] == $_SESSION['user_id'] || $_SESSION['user_role'] === 'admin')) {
            // Delete file from server
            if (file_exists($image_info['file_url'])) {
                unlink($image_info['file_url']);
            }
            
            // Delete from database
            $db->query("DELETE FROM gallery WHERE id = ?", [$image_id]);
        ...
        $delete_error = 'Failed to delete image. Please try again.';
    }
}
...
<body>
            ...
            <div class="gallery-grid" id="galleryGrid">
                <?php if ($gallery_images && $gallery_images->num_rows > 0): ?>
                    ...
                    <?php while ($image = $gallery_images->fetch_assoc()): ?>
                        <?php $galleryArray[] = $image; ?>
                        <div class="gallery-item" data-aos="fade-up" data-aos-delay="200">
                            <div class="gallery-actions">
                                <button class="gallery-action-btn" onclick="openLightbox(<?php echo count($galleryArray) - 1; ?>)">
                                    <i class="fas fa-expand"></i>
                                </button>
                                <?php if (isset($_SESSION['user_id']) && ($image['uploaded_by'] == $_SESSION['user_id'] || $_SESSION['user_role'] === 'admin')): ?>
                                    <form method="POST" style="display: inline;">
                                        <input type="hidden" name="image_id" value="<?php echo $image['id']; ?>">
                                        <input type="hidden" name="delete_image" value="1">
                                        <button type="submit" class="gallery-action-btn delete" onclick="return confirm('Are you sure you want to delete this image?')">
                                            <i class="fas fa-trash"></i>
                                        </button>
                                    </form>
                                <?php endif; ?>
                            </div>
                            <div class="gallery-image" onclick="openLightbox(<?php echo count($galleryArray) - 1; ?>)">
                                <img src="<?php echo htmlspecialchars($image['file_url']); ?>" alt="<?php echo htmlspecialchars($image['title'] ?? 'Gallery Image'); ?>" onerror="this.src='assets/hero-worship-CWyaH0tr.jpg'">
    ...
    <script>
        ...
        const galleryData = <?php echo json_encode($galleryArray ?? []); ?>;
        ...
        function openLightbox(index) {
            ...
            if (galleryData.length > 0 && index < galleryData.length) {
                lightboxImage.src = galleryData[index].file_url;
            } else {
                lightboxImage.src = sampleImages[index % sampleImages.length];
            }
        ...
        function navigateLightbox(direction) {
            const totalImages = galleryData.length > 0 ? galleryData.length : sampleImages.length;
            currentImageIndex = (currentImageIndex + direction + totalImages) % totalImages;
            
            const lightboxImage = document.getElementById('lightbox-image');
            if (galleryData.length > 0 && currentImageIndex < galleryData.length) {
                lightboxImage.src = galleryData[currentImageIndex].file_url;
            } else {
                lightboxImage.src = sampleImages[currentImageIndex % sampleImages.length];
            }
        }
