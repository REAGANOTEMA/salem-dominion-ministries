...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($first_name) && !empty($last_name) && !empty($email)) {
        try {
            $result = $db->update("UPDATE users SET first_name = ?, last_name = ?, email = ?, phone = ?, bio = ? WHERE id = ?", 
                                [$first_name, $last_name, $email, $phone, $bio, $_SESSION['user_id']]);
            ...
            if ($result > 0) {
                ...
                $update_error = 'No changes made to your profile.';
            }
