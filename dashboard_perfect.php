...
// Get dashboard data based on role with error handling
$dashboard_data = [];
$recent_activities = [];
...
try {
    switch ($user_role) {
        ...
        case 'pastor':
            $dashboard_data['upcoming_events'] = $db->select("SELECT * FROM events WHERE event_date >= CURDATE() ORDER BY event_date ASC LIMIT 5");
            ...
            $dashboard_data['recent_sermons'] = $db->select("SELECT * FROM sermons WHERE created_by = ? ORDER BY sermon_date DESC LIMIT 3", [$_SESSION['user_id']]);
            break;
            
        case 'member':
            $dashboard_data['upcoming_events'] = $db->select("SELECT * FROM events WHERE event_date >= CURDATE() ORDER BY event_date ASC LIMIT 5");
            $dashboard_data['recent_sermons'] = $db->select("SELECT * FROM sermons ORDER BY sermon_date DESC LIMIT 3");
            $dashboard_data['recent_news'] = $db->select("SELECT * FROM news WHERE status = 'published' ORDER BY created_at DESC LIMIT 3");
            $dashboard_data['my_prayer_requests'] = $db->select("SELECT * FROM prayer_requests WHERE created_by = ? ORDER BY created_at DESC LIMIT 3", [$_SESSION['user_id']]);
            break;
...
<head>
    ...
    <meta name="description" content="Member Dashboard - Salem Dominion Ministries">
...
<body>
    ...
    <aside class="sidebar" id="sidebar">
        ...
        <ul class="sidebar-menu">
            ...
            <?php endif; ?>
            <?php if ($user_role === 'pastor' || $user_role === 'admin'): ?>
                <li>
                    <a href="sermons.php">
    ...
    <main class="main-content">
        ...
        <header class="top-header">
            ...
            <h1 class="header-title">Welcome, <?php echo htmlspecialchars($user['first_name']); ?>!</h1>
            ...
            <div class="content-grid">
                ...
                <div class="content-card" data-aos="fade-up">
                    ...
                    <?php if (!empty($dashboard_data['upcoming_events'])): ?>
                        <?php foreach (array_slice($dashboard_data['upcoming_events'], 0, 3) as $event): ?>
                            <div class="list-item">
                                <div class="list-icon">
                                    <i class="fas fa-calendar"></i>
                                </div>
                                <div class="list-content">
                                    <div class="list-title"><?php echo htmlspecialchars($event['title']); ?></div>
                                    <div class="list-meta">
                                        <?php echo date('M j, Y', strtotime($event['event_date'])); ?> at <?php echo date('g:i A', strtotime($event['event_time'] ?? '10:00')); ?>
                                    </div>
                                </div>
                            </div>
                        <?php endforeach; ?>
                ...
                <div class="content-card" data-aos="fade-up" data-aos-delay="100">
                    <div class="card-header">
                        <h3 class="card-title">Recent Sermons</h3>
                        <a href="sermons.php" class="card-action">View All</a>
                    </div>
                    <?php if (!empty($dashboard_data['recent_sermons'])): ?>
                        <?php foreach (array_slice($dashboard_data['recent_sermons'], 0, 3) as $sermon): ?>
                            <div class="list-item">
                                <div class="list-icon">
                                    <i class="fas fa-microphone"></i>
                                </div>
                                <div class="list-content">
                                    <div class="list-title"><?php echo htmlspecialchars($sermon['title']); ?></div>
                                    <div class="list-meta">
                ...
                <?php if ($user_role === 'pastor' || $user_role === 'admin'): ?>
                    <!-- Prayer Requests (Pastor/Admin) -->
                    <div class="content-card" data-aos="fade-up" data-aos-delay="200">
                        <div class="card-header">
                            <h3 class="card-title">Prayer Requests</h3>
                            <a href="prayer_requests.php" class="card-action">View All</a>
                        </div>
                        <?php if (!empty($dashboard_data['prayer_requests'])): ?>
                            <?php foreach (array_slice($dashboard_data['prayer_requests'], 0, 3) as $request): ?>
                                <div class="list-item">
                                    <div class="list-icon">
                                        <i class="fas fa-praying-hands"></i>
                                    </div>
                                    <div class="list-content">
                                        <div class="list-title"><?php echo htmlspecialchars(substr($request['request_text'], 0, 50)); ?>...</div>
                                        <div class="list-meta">
                                            <?php echo date('M j, Y', strtotime($request['created_at'])); ?>
                                        </div>
                                    </div>
                                    <span class="list-badge">Pending</span>
                                </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="empty-state">
                ...
                <?php if ($user_role === 'member'): ?>
                    <!-- My Prayer Requests (Member) -->
                    <div class="content-card" data-aos="fade-up" data-aos-delay="200">
                        <div class="card-header">
                            <h3 class="card-title">My Prayer Requests</h3>
                            <a href="prayer_requests.php" class="card-action">View All</a>
                        </div>
                        <?php if (!empty($dashboard_data['my_prayer_requests'])): ?>
                            <?php foreach (array_slice($dashboard_data['my_prayer_requests'], 0, 3) as $request): ?>
                                <div class="list-item">
                                    <div class="list-icon">
                                        <i class="fas fa-praying-hands"></i>
                                    </div>
                                    ...
                                    <span class="list-badge"><?php echo ucfirst($request['status'] ?? 'pending'); ?></span>
                                </div>
                            <?php endforeach; ?>
                        <?php else: ?>
                            <div class="empty-state">
                ...
                <div class="content-card" data-aos="fade-up" data-aos-delay="300">
                    <div class="card-header">
                        ...
                        <a href="news.php" class="card-action">View All</a>
                    </div>
                    <?php if (!empty($dashboard_data['recent_news'])): ?>
                        <?php foreach (array_slice($dashboard_data['recent_news'], 0, 3) as $news): ?>
                            <div class="list-item">
                                <div class="list-icon">
                                    <i class="fas fa-newspaper"></i>
                                </div>
                                <div class="list-content">
                                    <div class="list-title"><?php echo htmlspecialchars($news['title']); ?></div>
                                    <div class="list-meta">
