    ...
    if ($file['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($file['type'], $allowed_types) && in_array($file_extension, $allowed_extensions)) {
            ...
            if (move_uploaded_file($file['tmp_name'], $filepath)) {
                if ($conn) {
                    $stmt = $conn->prepare("INSERT INTO gallery (uploaded_by, file_url, title, description, file_type, status) VALUES (?, ?, ?, ?, 'image', 'published')");
...
if ($conn) {
    try {
        $result = $conn->query("SELECT * FROM gallery WHERE status = 'published' ORDER BY created_at DESC");
        if ($result) {
            ...
            <div class="gallery-grid" id="galleryGrid">
                <?php if (!empty($gallery_images)): ?>
                    <?php foreach ($gallery_images as $index => $image): ?>
                        <div class="gallery-item" data-aos="fade-up" data-aos-delay="<?php echo 200 + ($index * 100); ?>">
                            ...
                            <div class="gallery-image" onclick="openLightbox(<?php echo $index; ?>)">
                                ...
                                <?php else: ?>
                                    <img src="assets/hero-worship-CWyaH0tr.jpg" alt="Gallery Image">
                                <?php endif; ?>
